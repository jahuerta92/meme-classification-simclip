...
from modules import (AttentionFeatureMatrixFuser, 
                     ForwardModuleList, 
                     LossBalancer, 
                     EMABalancer, 
                     CrossAttentionFuser, 
                     CrossAttentionFeatureFuser, 
                     AlignedSimilarityFuser,
                     SimilarityFuser,
                     TransformerFuser)
...
class DefaultModel(L.LightningModule):
    def __init__(self, encoder, class_weights, training_steps,
        ...
        self.class_weights = [torch.tensor(cw) for cw in class_weights]
        self.frozen = frozen
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_loss", torch.nanmean(torch.stack(losses)))
        return loss
    ...
    def test_step(self,  batch, batch_idx):
        ...
        for x_hat, y, in zip(x_hat_list,
                             unstacked_labels):
            x_p = x_hat.argmax(-1)
            x_hat_alt = x_hat[:, -1] if x_hat.shape[-1] == 2 else x_hat
            ...
            x_d = x_p.detach().cpu().numpy()[idx]
            x_h_d = F.softmax(
                x_hat_alt.detach().cpu().float(), dim=-1).numpy()[idx]
    ...
    def configure_optimizers(self):
        optimizer = Adafactor([
                ...
                {"params": self.projector.parameters(), "lr": self.learning_rate*10},
...
class CLIPv2Model(DefaultModel):
    def configure_transformer(self):
        self.transformer = AutoModel.from_pretrained(self.encoder,)
...
class CLIPCrossAttentionModel(DefaultModel):
    def configure_transformer(self):
        self.transformer = AutoModel.from_pretrained(self.encoder,)
        self.loss_balancer = EMABalancer(len(self.output_list), beta=.1)
...
class CLIPSimilarityModel(DefaultModel):
    def configure_transformer(self):
        self.transformer = AutoModel.from_pretrained(self.encoder,)
        self.loss_balancer = EMABalancer(len(self.output_list), beta=.1)
    ...
    def freeze(self):
        ...
        if self.frozen > 12:
            freeze_module(self.transformer.visual_projection)
            freeze_module(self.transformer.text_projection)

class CLIPSimilarityOnlyModel(DefaultModel):
    def configure_transformer(self):
        self.transformer = AutoModel.from_pretrained(self.encoder,)
        self.loss_balancer = EMABalancer(len(self.output_list), beta=.1)
...
class CLIPTransformerModel(DefaultModel):
    def configure_transformer(self):
        self.transformer = AutoModel.from_pretrained(self.encoder,)
        self.loss_balancer = EMABalancer(len(self.output_list), beta=.1)
        ...
        input_dim = 768
        hidden_dim = 64
        self.projector = TransformerFuser(input_dim)
        self.classifier = ForwardModuleList(nn.Sequential(nn.Linear(input_dim*2, hidden_dim),
                                                          nn.GELU(),
                                                          nn.Linear(hidden_dim, i)) for i in self.output_list)
